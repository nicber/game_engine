 ...
 */
template < typename T >
class aio_operation_t : public aio_operation_base, public std::enable_shared_from_this< aio_operation_t< T > >
{
public:
...
struct lambda_type< F, false >
{
  using type = typename std::remove_reference_t<
    std::tuple_element_t< 0, typename util::function_traits< F >::args > >::type;
};

template < typename F >
using lambda_type_cond_help = typename std::is_same< typename util::function_traits< F >::number_args,
