...
namespace render {
render_subsystem::render_subsystem( ) : update_thread_waiting( false )
{
}
...
void
render_subsystem::handle_events( )
{
  ...
  while ( true ) {
    {
      ...
      if ( !update_thread_waiting ) {
        cv.wait_for( lock, wait_time, [this] { return (bool) update_thread_waiting; } );
      }
...
bool
render_subsystem::has_exited( ) const
{
  return exited;
}

void
render_subsystem::update_all( )
{
...
bool
render_subsystem::update_drawers_if_nec( )
{
  ...
  if ( update_thread_waiting ) {
    for ( auto& components_of_a_type : reg_components ) {
      ...
      for ( auto comp : vector_of_components ) {
        auto drawer       = dynamic_cast< render_component* >( comp )->update_opengl_get_drawer( );
