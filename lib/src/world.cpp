...
namespace game_engine
{
	namespace logic
	{
		...
		void game::add_entity(std::unique_ptr<entity> ent_ptr)
		{
			auto& ent = *ent_ptr;
		...
		bool game::add_component(component& comp)
		{
			for (auto& subsys_ptr : subsystems)
			{
		...
		void game::remove_entity(entity& ent)
		{
			...
			auto it = std::lower_bound(ent_vector.begin(), ent_vector.end() - unsort_num, &ent,
				[](std::unique_ptr<entity>& lhs, entity* rhs){ return lhs.get() < rhs; });
			if (it == ent_vector.end() - unsort_num)
			{
				std::find_if(it, ent_vector.end(), [&ent](std::unique_ptr<entity>& ptr){ return ptr.get() == &ent; });
			}
