...
struct check_not_stencil_or_depth : public boost::static_visitor< bool >
{
  ...
  {
    return at != attachment::stencil_attachment && at != attachment::depth_attachment &&
...
void
output_location_set::connect( connection_dest dest, GLuint location )
{
...
connection_dest
output_location_set::get_connection( GLuint loc ) const
{
...
output_location_set::get_connection( connection_dest dest ) const
{
...
bool
output_location_set::remove_connection( GLuint loc )
{
...
bool
output_location_set::remove_connection( connection_dest dest )
{
  auto it = find_equal_dest( dest );
  if ( it != connections.end( ) ) {
...
void
output_location_set::apply_to( framebuffer& fb ) const
{
...
void
output_location_set::update_vector_if_nec( ) const
{
  if ( vector_uptodate ) {
    return;
  }
  ...
  if ( connections.empty( ) ) {
    ...
    attachments_in_order.clear( );
    std::fill_n( std::back_inserter( attachments_in_order ), last + 1, GL_NONE );
...
{
  auto it = std::find_if(
    connections.cbegin( ), connections.cend( ), [&dest]( auto& pair ) { return pair.second == dest; } );
  return it;
}
}
}
