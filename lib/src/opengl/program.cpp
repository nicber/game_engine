...
namespace opengl {
program_construction_error::program_construction_error( std::string error, const shader&, const shader&,
...
program_uniform_block_binding_manager::program_uniform_block_binding_manager( program* ptr ) : prog_ptr( ptr )
{
}

static program::uniform_cont::nth_index< 0 >::type::const_iterator
find_block_by_name( const program& prog, const std::string& name )
{
  prog.bind( );
...
static auto
find_binding_by_name( const program_uniform_block_binding_manager::handles_vector& hv,
                      const std::string& binding_name )
{
  auto it = std::find_if( hv.begin( ), hv.end( ), [&binding_name]( decltype( hv[ 0 ] )& handle ) {
...
void
program_uniform_block_binding_manager::add_binding( const std::string& block_name,
                                                    const std::string& binding_name )
{
...
void
program_uniform_block_binding_manager::remove_binding_by_binding_name( const std::string& binding_name )
{
...
bool
program_uniform_block_binding_manager::check_compatibility( ) const
{
...
bool
program_uniform_block_binding_manager::check_compatibility_binding( const std::string& binding_name ) const
{
  ...
  do {
    auto same_name_it = std::lower_bound(
      buffer_variables.cbegin( ), buffer_variables.cend( ), *block_it, uniform_buff_variable_sort_func );
  ...
  } while ( ++block_it != prog_ptr->uniforms.get< 0 >( ).cend( ) && block_it->block_name == bind_it->first );
  return true;
}

program::program( const shader& s1, const shader& s2, std::initializer_list< shader > shaders,
                  ...
                  std::initializer_list< frag_data_loc > frag_data )
{
  ...
  if ( vertex_count == 0 ) {
    throw program_construction_error( "no vertex shaders", s1, s2, shaders, attribs, frag_data );
  } else if ( fragment_count == 0 ) {
    throw program_construction_error( "no fragment shaders", s1, s2, shaders, attribs, frag_data );
  }
  ...
  for ( auto& a_p : attribs ) {
    glBindAttribLocation( program_id, std::get< 1 >( a_p ), std::get< 0 >( a_p ).c_str( ) );

    vertex_attrs.emplace( std::move( std::get< 0 >( a_p ) ), vertex_attr( std::get< 1 >( a_p ) ) );
  }

  for ( auto& f_d : frag_data ) {
    glBindFragDataLocation( program_id, std::get< 1 >( f_d ), std::get< 0 >( f_d ).c_str( ) );

    frag_locs.emplace( std::move( std::get< 0 >( f_d ) ), frag_loc( std::get< 1 >( f_d ) ) );
  }
  ...
  if ( !compiled ) {
    ...
    throw program_construction_error( std::string( log.data( ) ), s1, s2, shaders, attribs, frag_data );
  }
...
bool
program::has_vertex_attr( const std::string& name ) const
{
...
const vertex_attr&
program::get_vertex_attr( const std::string& name ) const
{
  auto it = find_vertex_attr( name );
  return it->second;
}
...
bool
program::has_uniform( const std::string& name ) const
{
...
const program::uniform_cont&
program::get_uniforms( ) const
{
...
bool
program::has_frag_loc( const std::string& name ) const
{
...
const frag_loc&
program::get_frag_loc( const std::string& name ) const
{
  auto it = find_frag_loc( name );
  return it->second;
}
...
typename C::iterator
find_generic( GLuint program_id, const std::string& name, C& cont, F func )
{
...
program::unor_map_i< vertex_attr >
program::find_vertex_attr( const std::string& name ) const
{
  return find_generic( program_id, name, vertex_attrs, glGetAttribLocation );
}

program::unor_map_i< frag_loc >
program::find_frag_loc( const std::string& name ) const
{
  return find_generic( program_id, name, frag_locs, glGetFragDataLocation );
}

program::uniform_cont::nth_index< 1 >::type::const_iterator
program::find_uniform( const std::string& name ) const
{
  setup_uniforms_if_nec( );
  return uniforms.get< 1 >( ).find( name );
}

program_uniform_block_binding_manager&
program::ubb_manager( )
{
...
const program_uniform_block_binding_manager&
program::ubb_manager( ) const
{
  assert( bind_manager.prog_ptr == this );
  return bind_manager;
}
...
void
program::setup_uniforms_if_nec( ) const
{
