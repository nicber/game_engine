...
prepare_vector( GLsizei count, unsigned components, const bool* data )
{
  std::vector< int > ret;
...
void
gl_uniform1bv( GLint location, GLsizei count, const bool* data )
{
...
void
gl_uniform2bv( GLint location, GLsizei count, const bool* data )
{
  auto vec( prepare_vector( count, 1, data ) );
...
void
gl_uniform3bv( GLint location, GLsizei count, const bool* data )
{
  auto vec( prepare_vector( count, 1, data ) );
...
void
gl_uniform4bv( GLint location, GLsizei count, const bool* data )
{
  auto vec( prepare_vector( count, 1, data ) );
  ...
  static std::unordered_map< GLenum, names_struct > names_map = [] {
    ...
    for ( size_t i( 0 ); i < names_array_counter; ++i ) {
      ret.emplace( names_array[ i ].enum_val,
                   names_struct{ names_array[ i ].cpp_name, names_array[ i ].enum_name } );
    }
    return ret;
  }( );
  return names_map;
...
bool
operator==( const uniform& lhs, const uniform& rhs )
{
#define COMP( x ) ( lhs.x == rhs.x )
  return COMP( name ) && COMP( block_name ) && COMP( block_index ) && COMP( block_size ) &&
         COMP( block_offset ) && COMP( array_size ) && COMP( block_array_stride ) &&
         COMP( block_matrix_stride ) && COMP( matrix_row_major ) && COMP( type );
#undef COMP
}
...
std::vector< uniform_setter >
get_uniforms_of_program( GLuint prog_id )
{
  ...
  auto get_data = [prog_id, &uniform_indices, number_uniforms]( GLenum param ) {
    ...
    glGetActiveUniformsiv( prog_id, number_uniforms, uniform_indices.data( ), param, data_retrieved.data( ) );
  ...
  if ( number_blocks != 0 ) {
    ...
    size_t max_block_name_length = *std::max_element( name_lengths.begin( ), name_lengths.end( ) );
    std::vector< char > block_name_buffer( max_block_name_length );

    for ( size_t i = 0; i < (size_t) number_blocks; ++i ) {
      GLsizei real_length;
      glGetActiveUniformBlockName(
        prog_id, i, max_block_name_length, &real_length, block_name_buffer.data( ) );
...
const std::string&
uniform_block_binding::get_name( ) const
{
  return name;
}

GLint
uniform_block_binding::get_id( ) const
{
  return id;
}

std::shared_ptr< uniform_buffer_data >
uniform_block_binding::get_bound_buffer_data( ) const
{
  return bound_buffer_data.lock( );
}

const std::string&
get_name( const std::shared_ptr< uniform_block_binding >& ptr )
{
  return ptr->get_name( );
}

/*
 * The uniform block binding manager uses shared_ptrs to keep track of the
 * bindings it
 * allocates for users. When a binding is no longer being used, its use_count()
 * drops to 1,
 * which tells us that we are free to reuse it if we need to. If a binding the
 * name passed as
 * the parameter has already been allocated, then that binding is returned again
 * and, therefore,
...
using binding_container = boost::multi_index_container<
  ...
  boost::multi_index::indexed_by<
    ...
    boost::multi_index::ordered_non_unique< boost::multi_index::global_fun<
      const std::shared_ptr< uniform_block_binding >&, const std::string&, &get_name > > > >;

uniform_block_binding_handle
get_free_uniform_block_binding( std::string name )
{
