...
namespace opengl {
namespace parser {
inconsistent_file::inconsistent_file( std::string error_msg_, std::vector< face > faces_,
...
inconsistent_file::get_faces( ) const
{
  return faces;
}

const std::vector< glm::vec3 >&
inconsistent_file::get_vertices( ) const
{
  return vertices;
}

const std::vector< glm::vec3 >&
inconsistent_file::get_tex_coords( ) const
{
  return tex_coords;
}

const std::vector< glm::vec3 >&
inconsistent_file::get_normals( ) const
{
  return normals;
}

static std::string
create_message( const std::string& message_str, const std::string& source_str, size_t pos )
{
  size_t context_start( pos >= 10 ? pos - 10 : 0 );
  std::stringstream ss;

  ss << "parsing failure aproximately at position " << pos << " : " << message_str << " in source:\n"
...
const std::string&
parse_error::error_message( ) const
{
  return message_str;
}

const std::string&
parse_error::source_string( ) const
{
  return source_str;
}

size_t
parse_error::position( ) const
{
  return pos;
}

static size_t
hash_value( const v_vt_n_indices& i )
{
  size_t seed( 0 );
...
static bool
operator==( const v_vt_n_indices& lhs, const v_vt_n_indices& rhs )
{
  bool r = lhs.v_i == rhs.v_i && lhs.vt_i == rhs.vt_i && lhs.n_i == rhs.n_i;
  if ( r ) {
...
static obj_model
create_indices( std::vector< face > faces, std::vector< glm::vec3 > vertices,
                std::vector< glm::vec3 > tex_coords, std::vector< glm::vec3 > normals )
{
  unsigned int id( 1 );
  ...
  for ( auto& f : faces ) {
    for ( auto& ind : f.indices ) {
      ...
      // we have to count from 1 so that we can tell which indices are new (i.e.
      // == 0),
  ...
  return {
    std::move( new_indices ), std::move( new_vertices ), std::move( new_tex_coords ), std::move( new_normals )
  };
}

obj_model
parse_obj_format( const char* str )
{
  ...
  while ( *str ) {
    ...
    if ( std::strcmp( "f", def_type ) == 0 ) {
      ...
      unsigned int v_i[ 3 ], vt_i[ 3 ], n_i[ 3 ];

      if ( std::sscanf( str + pos,
                        "f %u/%u/%u %u/%u/%u %u/%u/%u%n",
                        &v_i[ 0 ],
                        &vt_i[ 0 ],
                        &n_i[ 0 ],
                        &v_i[ 1 ],
      ...
      glm::vec3 vt;
      auto matches = std::sscanf(
        str + pos, "vt %f %f%n %f%n", &vt.x, &vt.y, &delta_pos_without_z, &vt.z, &delta_pos_with_z );
...
obj_model
parse_obj_format( const std::string& str )
{
